...
Given /^a patient visit$/ do
  @fake_visit = { #:session_id => @fake_session[:session][:id]
                  :patient_visit => { :chapter_id => "yourmom",
                                     :choices => [
...
When /^I send the patient visit to the patient visit controller$/ do
  ...
  @response = RestClient.post "localhost:3000/sessions/#{@fake_session[:session][:id]}/patient_visits", @fake_visit_as_json, :content_type => :json, :accept => :json
end
